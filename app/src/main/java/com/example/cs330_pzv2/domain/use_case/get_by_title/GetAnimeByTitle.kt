package com.example.cs330_pzv2.domain.use_case.get_by_title
...
class GetAnimeByTitle @Inject constructor(
    private val repository: AnimeRepository
) {
    operator fun invoke(title: String) : Flow<Resource<List<Anime>>> = flow {

        try {
            emit(Resource.Loading<List<Anime>>())
            val animes = repository.getAnimeByTitle(title).map {
                it.toAnime()
            }
